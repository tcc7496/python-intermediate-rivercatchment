...
def main(args):
    ...
    if args.full_data_analysis:
        _, ext = os.path.splitext(InFiles[0])
        if ext == ".csv":
...
def create_argparse():
    # have this in a function so we can create some tests.
    # also good practise for separating concerns
    parser = argparse.ArgumentParser(
        description='A basic environmental data management system')
    
    req_group = parser.add_argument_group('required arguments') # not a necessity, but makes it easier for end users
